...
const GET_CATALOG_PRODUCT_ITEMS_TOOL: Tool = {
  ...
  description: "Lista TODOS os sellers que vendem aquele produto do catalogo, com price/condition/warranty/listing_type/full_fulfillment. Default limit=20 (max 100). USE QUANDO: quer comparar precos entre sellers do MESMO produto canonico. Use enrich_seller=true pra incluir reputation/transactions/permalink inline (chamadas paralelas a /users/{seller_id}; evita N+1 get_seller_info quando voce ja precisa avaliar reputacao). Cada item retorna `tags` (slugs crus do ML) e `tags_decoded` ({is_official_brand_store, requires_immediate_payment, supports_cart} — booleans decodificados pras 3 tags acionaveis pra recomendacao). Slugs conhecidos em `tags`: brand_verified (loja oficial verificada da marca), cart_eligible (compra via carrinho), immediate_payment (pagamento obrigatorio no checkout), good_quality_thumbnail (ML validou imagem principal), standard_price_by_quantity (preco escalonado por volume), kvs_primary (anuncio principal do produto na busca), dynamic_standard_price (preco dinamico ajusta com demanda/concorrencia), has_published_clips (anuncio tem videos), user_product_unify (anuncio unificado de varias listings), best_seller_candidate (forte candidato a destaque na categoria), incomplete_technical_specs (atributos tecnicos faltando), supermarket_eligible (elegivel mercado online/supermercado).",
  inputSchema: {
    type: "object",
    properties: {
      catalog_id: { type: "string", description: "Catalog product ID (ex: MLB1027172667)" },
